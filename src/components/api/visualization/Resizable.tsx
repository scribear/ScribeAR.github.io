...
function drop(event) {
    ...
    if (!resizableElem) return
    resizableElem.style.left = (event.clientX + parseInt(offset[0],10)) + 'px';
    resizableElem.style.top = (event.clientY + parseInt(offset[1],10)) + 'px';
    event.preventDefault();
    return false;
} 

const setVisual = (id) => {
    resizableElem = document.getElementById(id)
